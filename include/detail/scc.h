...
void tarjan_iterative(std::vector<Vertex>& vertices, std::vector<VertexIndex>& scc, bool ignore_labels = false)
{
	...
	for (size_t i = 0; i < vertices.size(); ++i)
	{
		...
		while (not stack.empty())
		{
			...
			if (low[vi] == 0 and scc[vi] == 0)
			{
				...
				{
					if ((low[*w] == 0) and (scc[*w] == 0) and (ignore_labels || (vertices[*w].label == v.label)))
						stack.push_front(*w);
				}
			}
			else
			{
				for (graph::VertexSet::const_iterator w = v.out.begin(); w != v.out.end(); ++w)
				{
					if ((low[*w] != 0) and (ignore_labels || (vertices[*w].label == v.label)))
...
template <typename Vertex>
void guarantee_vertex0_in_scc0(std::vector<Vertex>& vertices, std::vector<VertexIndex>& sccs)
{
...
void collapse(std::vector<Vertex>& vertices, std::vector<VertexIndex>& sccs)
{
  ...
  guarantee_vertex0_in_scc0(vertices, sccs);
